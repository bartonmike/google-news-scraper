...
Article that explains this method: https://medium.com/daily-python/python-script-to-edit-google-sheets-daily-python-7-aadce27846c0 
...
The google sheets that this program makes is formatted as follows:
...
--second sheet (Named "Prompts")
    ...
    cell A3 - cell that stores the prompt templates
    cell C3 - cell that stores the affiliation definitions
    cell A5 - cell that stores the custom prompts
    cell C5 - cell that stores the date 

--third sheet (Named "Prompt_history")
    first row - stores the titles of the table ('Prompt Changes', 'URL')
...
# Prompt Formatting
  ...
  exact phrases, so the prompt templates must be in quotation markes. Each line denotes one individual search, a search can 
  contain multiple quotation mark queries "such as this" "with this". 
      How the template takes values from the people is it uses keywords to be able to insert the information of the person. 
  This program takes in a persons first name, last name, and affiliation to be able to search news for them, a template should include those
  inside so that the program can properly search for them. 
      when you write, 'First Name', 'Last Name', and 'Affiliation' into the template it will replace them with the relative data for the person. 
  An example for prompt formatting is as follows: 
...
# Affiliations
  ...
  How these inputs work is through an variable/assignment type statement. You input the affiliation that you would put in the affiliations column in the 
  person list, type an equals, then the alternate definition of such affiliation, for example "OSU = Oregon State University". If you want to add another 
...
Prompts person/theme separation formatting: 
  ...
  if you are searching through multiple subjects, each with their own bundles of search lines, it is best to separate them 
...
Can't Double Check Prompt in Article::: This is the most common note that one might see in the program. The cause of this note can be many different reasons, but here are a few common ones.
    ...
    - The article only reaveals a portion of the text and needs the user to press a button to reveal the rest, with the search term in that hidden part (Common for MSN news)
    - The article is formatted in such a way that the program cannot pull the text and do a check

Timed Out::: This error is pretty much a blanket statement for any errors that the program encountered trying to open and fetch data from an article.
...
go to website to create project, key, and access api's: https://console.developers.google.com/ , create a project, download Drive and Sheets apis, 
